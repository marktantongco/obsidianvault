---
title: Wiki Index
updated: 2026-07-09T09:58:13Z
---

# Wiki Index

*Last updated: 2026-07-09T09:58:13Z*

## Concepts
- [[concepts/AI Agents]] — Core agent concepts and architecture
- [[concepts/OpenCode]] — Terminal-based AI coding agent
- [[concepts/Hermes Agent]] — Self-improving orchestrator
- [[concepts/Claude Code]] — Anthropic agentic coding tool
- [[concepts/MiMo Code]] — Xiaomi AI coding assistant
- [[concepts/OpenAI Codex]] — OpenAI coding agent
- [[concepts/Grok]] — xAI assistant / Grok Build
- [[concepts/Obsidian Wiki]] — Local markdown second brain
- [[concepts/multi-agent-obsidian-wiki-synergy]] — Shared vault + agent wiring ( #core )
- [[concepts/Development Workflow]] — Multi-agent development process
- [[concepts/MCP Servers]] — Model Context Protocol tools

## Entities
- [[entities/Owl-Agent]] — CN-aware resilient HTTP proxy client
- [[entities/GPaste]] — GNOME clipboard manager
- [[entities/CopyQ]] — Qt clipboard (autostart off)
- [[entities/SiliconFlow]] — China AI inference API hub
- [[entities/Zhipu-GLM]] — Zhipu GLM APIs / free Flash
- [[entities/DeepSeek]] — DeepSeek LLM APIs

## Skills
- [[skills/free-china-proxy-ops]] — Direct-first CN proxy procedure
- [[skills/gpaste-gnome-wayland]] — GPaste install/dock/shortcuts
- [[skills/multi-agent-vault-wiring]] — Symlinks + AGENTS bootstrap

## References
- [[references/free-china-proxy-research]] — Source research note
- [[references/ai-skills-consolidated-framework]] — 67-skill catalogue source
- [[references/obsidian-claude-memory-framework]] — Karpathy wiki protocol for Claude

## Synthesis
- [[synthesis/agentic-stack-obsidian-wiki-performance]] — System performance assessment

## Journal
- [[journal/2026-07-09-multi-agent-wiki-compound]] — Compound session log
- [[journal/2026-07-13-second-brain-integration-session]] — Second brain full stack integration (v5.1.1, MiMo, scripts, vault restructure)

## Projects
- [[projects/AI-Second-Brain]] — Multi-agent AI setup project
- [[projects/Cross-Agent-Workflow-Test]] — Cross-agent workflow test

## Configuration
- [[config/README|System Config]]
- [[config/copyq/README|CopyQ Setup]]

## Special
- [[hot]] · [[log]] · [[AGENTS]]
